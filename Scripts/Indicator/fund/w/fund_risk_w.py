    ...
    for month in range(1, 1+1):
        ...
        for mday in range(1, month_range + 1):
            ...
            for i in range(len(ids_used)):
                ...
                stdev = [fi.standard_deviation(r) for r in rs3]
                stdev_a = [fi.standard_deviation_a(r) for r in rs3]
                dd_a = [fi.downside_deviation_a(r, r_f) for r, r_f in zip(rs3, rs_f3_)]
                mdd = [fi.max_drawdown(nv)[0] for nv in navs3]
                ...
                for k in benchmark.keys():
                    ...
                    beta = [fi.beta(r, r_bm, r_f) for r, r_bm, r_f in zip(rs3, rs_bm3, rs_f3_)]
                    corr = [fi.corr(r, r_bm)[0] for r, r_bm in zip(rs3, rs_bm3)]

                    tmp = [stdev, stdev_a, dd_a, mdd, beta, corr]
        ...
        df[list(range(3, 57))] = df[list(range(3, 57))].astype(np.float64)
        df[list(range(3, 57))] = df[list(range(3, 57))].apply(lambda x: round(x, 6))
        df.columns = columns
...
#     sql_dup = sql_dup[:-2]
#     conn.execute("INSERT INTO {0} {1} VALUES {2} ON DUPLICATE KEY UPDATE {3}".format(table2.name, sql_col, record_used, sql_dup))
# conn.close()
