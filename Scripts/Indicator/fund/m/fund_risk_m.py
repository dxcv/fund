    ...
    for month in range(2, 3):
        ...
        for mday in range(3, 4):
            ...
            for i in range(len(ids_used)):
                ...
                stdev = [fi.standard_deviation(r) for r in rs2]
                stdev_a = [fi.standard_deviation_a(r, 12) for r in rs2]
                dd_a = [fi.downside_deviation_a(r, r_f, 12) for r, r_f in zip(rs2, rs_f2_)]
                mdd = [fi.max_drawdown(nv)[0] for nv in navs2]
                ...
                for k in benchmark.keys():
                    ...
                    beta = [fi.beta(r, r_bm, r_f) for r, r_bm, r_f in zip(rs2, rs_bm2, rs_f2_)]
                    corr = [fi.corr(r, r_bm)[0] for r, r_bm in zip(rs2, rs_bm2)]
        ...
        df[list(range(3, 51))] = df[list(range(3, 51))].astype(np.float64)
        df[list(range(3, 51))] = df[list(range(3, 51))].apply(lambda x: round(x, 6))
        df.columns = columns
