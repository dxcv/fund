...
for year in range(2017, 2018):
    for month in range(2, 3):
        ...
        for mday in range(3, 4):
            ...
            date_s = sf.Time(dt.datetime(year, month, mday) - dt.timedelta(mday))  # Generate statistic_date

            sql_ids_updated = sf.SQL.ids_updated_sd(date_s.today, freq="m")
        ...
        for i in range(len(ids_used)):
            ...
            for j in range(7):
                if t_mins[i] + relativedelta(months=intervals[j]) <= date_s.today:
                    ...
                    navs.append(nav_matchs_all[i][:t_stds_len[j]])
                    ts.append(t_matchs_all[i][:t_stds_len[j]])
                else:
                    navs.append(None)
                    ts.append(None)
            ...
            ts.append(t_matchs_all[i])
            navs2 = [navs_[i] for i in intervals2]
            rs2 = [fi.gen_return_series(x) for x in navs2]
            ts2 = [ts[i] for i in intervals2]

            con_rise_weeks = [fi.periods_continuous_rise(r)[0] for r in rs2]
            con_fall_weeks = [fi.periods_continuous_fall(r)[0] for r in rs2]
            p_earning_weeks = [fi.periods_positive_return(r) for r in rs2]
            n_earning_weeks = [fi.periods_npositive_return(r) for r in rs2]
            ...
            for k in benchmark.keys():
                ...
                s_time = [fi.competency_timing(r, r_bm, r_f) for r, r_bm, r_f in zip(rs2, rs_bm2, rs_f2_)]
                s_security = [fi.competency_stock(r, r_bm, r_f) for r, r_bm, r_f in zip(rs2, rs_bm2, rs_f2_)]
                persistence = [fi.persistence_er(r, r_bm) for r, r_bm in zip(rs2, rs_bm2)]
                odds = [fi.odds(r, r_bm) for r, r_bm in zip(rs2, rs_bm2)]
                unsys_risk = [fi.unsystematic_risk(r, r_bm, r_f) for r, r_bm, r_f in zip(rs2, rs_bm2, rs_f2_)]
                tracking_error_a = [fi.tracking_error_a(r, r_bm, 12) for r, r_bm in zip(rs2, rs_bm2)]
                rvalue = [fi.value_at_risk(r) for r in rs2]
                min_return = [fi.min_return(r)[0] for r in rs2]
                max_return = [fi.max_return(r)[0] for r in rs2]
                mdd_ = [fi.max_drawdown(nv, t)[1] for nv, t in zip(navs2, ts2)]
                # mdd_ = [DF.max_drawdown(nv)[1] for nv in navs2]

                mdd_repair_time = [x[1] if x is not None else None for x in mdd_]
                mdd_time = [x[0] if x is not None else None for x in mdd_]
                skewness = [fi.skewness(r) for r in rs2]
                kurtosis = [fi.kurtosis(r) for r in rs2]
                ERVaR = [fi.ERVaR(r, r_f) for r, r_f in zip(rs2, rs_f2_)]
        ...
        df[list(range(3, 147))] = df[list(range(3, 147))].astype(np.float64)
        df[list(range(3, 147))] = df[list(range(3, 147))].apply(lambda x: round(x, 6))
        df.columns = columns
...
io.to_sql(table.name, engine_wt, df, chunksize=5000)


# df = df_whole
# df = df.sort_values(by=["fund_id", "statistic_date", "benchmark"], ascending=[True, False, True])
# df = df.drop_duplicates(subset=["fund_id", "benchmark"])

# conn = engines["1Gb"].connect()
# table2 = du.Table("m", "indicator")
# columns2 = ["fund_id", "statistic_date", "benchmark", "update_time"]
# columns2.extend(table2.columns())
# tmp = df[[x for x in columns if x in columns2]]
# del tmp["update_time"]
# tmp["statistic_date"] = tmp["statistic_date"].apply(lambda x: str(x))
# cols = np.array(tmp.columns)
# tmp_matrix = tmp.as_matrix()
#
# for record in tmp_matrix:
#     idx = np.insert(np.isnan(list(record[2:])), 0, [False, False])
#     record_used = tuple(record[~idx])
#     cols_used = tuple(cols[~idx])
#     sql_col = str(cols_used).replace("'", "`")
#     sql_dup = ""
#     for x in cols_used:
#         sql_dup += "`{0}` = VALUES(`{0}`), ".format(x)
#     sql_dup = sql_dup[:-2]
#     conn.execute(
#         "INSERT INTO {0} {1} VALUES {2} ON DUPLICATE KEY UPDATE {3}".format(table2.name, sql_col, record_used, sql_dup))
# conn.close()
