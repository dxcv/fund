...
def calculate(idx, export_path=None):
    ...
    tag = tu.timeseries_std(dt.datetime(year, month, day),
                            tu.periods_in_interval(dt.datetime(year, month, day),
                                                   dt.datetime(first_date.year, 1, 1),
                                                   12),
                            52, extend=5)[::-1]
    ...
    if export_path is not None:
        ...
        io.export_to_xl(components, "{sd}_{index_name}_w_samples".format(sd=last_monday.strftime("%Y%m%d"), index_name=PEIndex.id), export_path)
...
if __name__ == "__main__":
    main("/home/yusy/log/index")
    # main(su.get_desktop_path())
