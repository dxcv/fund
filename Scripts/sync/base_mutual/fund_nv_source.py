...
def fetch_multisource_nv(update_time_l, update_time_r=None):
    ...
    if update_time_r is None:
        condition = and_(DFundNv.update_time >= update_time_l, IdMatch.is_used == 1)
    else:
        condition = and_(DFundNv.update_time >= update_time_l, DFundNv.update_time <= update_time_r, IdMatch.is_used == 1)
