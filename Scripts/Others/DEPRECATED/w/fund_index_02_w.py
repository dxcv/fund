...
for year in range(first_date.timetuple().tm_year, now.timetuple().tm_year+1):
    ...
    AND fund_id IN (SELECT fund_id FROM fund_type_mapping WHERE type_code = 60401) \
...
tag = TU.timeseries_std(dt.datetime(year, month, day), TU.periods_in_interval(dt.datetime(year, month, day), dt.datetime(first_year, 1, 1), 12), 52, extend=5)[::-1]
...
#op.to_csv("C:\\Users\\Kael Yu\\Desktop\\fund_index_02_w.csv", index=False)
...
for i in range(len(tag)):
    res_tmp = {"index_id": "FI02", "index_name":"阳光私募指数", "statistic_date":str(tag[i]), "update_time":now_str,
               "typestandard_code": "604", "typestandard_name": "按发行主体分类", "type_code": "60401", "type_name": "信托",
