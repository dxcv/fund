...
for year in range(first_date.timetuple().tm_year, now.timetuple().tm_year+1):
    ...
    AND fund_id IN (SELECT fund_id FROM fund_type_mapping WHERE type_code = 60107) \
...
#op.to_csv("C:\\Users\\Kael Yu\\Desktop\\fund_index_13_w.csv", index=False)
...
for i in range(len(tag)):
    res_tmp = {"index_id": "FI13", "index_name":"组合投资策略私募指数", "statistic_date":str(tag[i]), "update_time":now_str,
               "typestandard_code": "601", "typestandard_name": "按投资策略分类", "type_code": "60107", "type_name": "组合策略",
