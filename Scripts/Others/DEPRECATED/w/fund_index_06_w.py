...
for year in range(first_date.timetuple().tm_year, now.timetuple().tm_year+1):
    ...
    AND fund_id IN (SELECT fund_id FROM fund_type_mapping WHERE stype_code = 6010103) \
    ORDER BY fund_id ASC, statistic_date DESC \
    ".format(dt.date(year, 12, 31), dt.date(year-1, 12, 18))

    sql_mindate = "\
    SELECT fund_id, MIN(statistic_date) as statistic_date_earliest FROM fund_nv_data WHERE fund_id IN \
    (SELECT fund_id FROM fund_nv_data WHERE statistic_date <= '{0}' and statistic_date >= '{1}' \
    AND nav IS NOT NULL AND statistic_date IS NOT NULL \
    AND fund_id IN (SELECT fund_id FROM fund_type_mapping WHERE stype_code = 6010103) \
    ...
    t_std = T.timeseries_std(last_monday, "y", 52, extend=1)    #
    if year == first_date.timetuple().tm_year: t_std = t_std[:-1]

    #
    tic("Slicing")
    t_std_long = T.timeseries_std(last_monday, T.periods_in_interval(last_monday, dt.date(year-1,11,30), 12))
    t_std_long_p1m = [(x + relativedelta(months=1)).timestamp() for x in T.tr(t_std_long)]
    ...
    tic("Matching...")
    matchs = [T.outer_match4index_w(x, t_std, False) for x in ts]
...
tag = T.timeseries_std(dt.datetime(year, month, day), T.periods_in_interval(dt.datetime(year, month, day), dt.datetime(first_year, 1, 1), 12), 52, extend=5)[::-1]
...
#op.to_csv("C:\\Users\\Kael Yu\\Desktop\\fund_index_06_w.csv", index=False)
...
for i in range(len(tag)):
    res_tmp = {"index_id": "FI06", "index_name":"市场中性策略私募指数", "statistic_date":str(tag[i]), "update_time":now_str,
               "typestandard_code": "601", "typestandard_name": "按投资策略分类", "type_code": "60101", "type_name": "股票策略",
               "stype_code":"6010103", "stype_name":"市场中性", "index_method": "1", "data_source":"0", "data_source_name":"私募云通",
