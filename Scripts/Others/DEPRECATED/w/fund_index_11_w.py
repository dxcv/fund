...
for year in range(first_date.timetuple().tm_year, now.timetuple().tm_year+1):
    ...
    AND fund_id IN (SELECT fund_id FROM fund_type_mapping WHERE type_code = 60103) \
...
#op.to_csv("C:\\Users\\Kael Yu\\Desktop\\fund_index_11_w.csv", index=False)
...
for i in range(len(tag)):
    res_tmp = {"index_id": "FI11", "index_name":"相对价值策略私募指数", "statistic_date":str(tag[i]), "update_time":now_str,
               "typestandard_code": "601", "typestandard_name": "按投资策略分类", "type_code": "60103", "type_name": "相对价值",
