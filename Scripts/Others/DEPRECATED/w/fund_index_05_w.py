...
for year in range(first_date.timetuple().tm_year, now.timetuple().tm_year+1):
    ...
    AND fund_id IN (SELECT fund_id FROM fund_type_mapping WHERE stype_code = 6010102) \
...
#op.to_csv("C:\\Users\\Kael Yu\\Desktop\\fund_index_05_w.csv", index=False)
...
for i in range(len(tag)):
    res_tmp = {"index_id": "FI05", "index_name":"股票多空策略私募指数", "statistic_date":str(tag[i]), "update_time":now_str,
               "typestandard_code": "601", "typestandard_name": "按投资策略分类", "type_code": "60101", "type_name": "股票策略",
               "stype_code":"6010102", "stype_name":"股票多空", "index_method": "1", "data_source":"0", "data_source_name":"私募云通",
