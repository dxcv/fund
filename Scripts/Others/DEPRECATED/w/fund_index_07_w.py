...
for year in range(first_date.timetuple().tm_year, now.timetuple().tm_year+1):
    ...
    AND fund_id IN (SELECT fund_id FROM fund_type_mapping WHERE type_code = 60105) \
...
#op.to_csv("C:\\Users\\Kael Yu\\Desktop\\fund_index_07_w.csv", index=False)
...
for i in range(len(tag)):
    res_tmp = {"index_id": "FI07", "index_name":"市场中性策略私募指数", "statistic_date":str(tag[i]), "update_time":now_str,
               "typestandard_code": "601", "typestandard_name": "按投资策略分类", "type_code": "60105", "type_name": "债券策略",
