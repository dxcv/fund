...
interval = ["m","s","y",1,3,6,12,24,36,60]
#interval_ = ["m", 1, "s", 3, 6, 12, "y", 24, 36, 60]
#interval_2 = ["y",1,3,6,12,24,36,60]
interval_3 = ["y",3,6,12,24,36,60]
variable_name1 = ["ir", "r_a", "er_a"]
variable_name3 = ["calmar_a", "sortino_a", "info_a", "jensen_a", "treynor_a"]
...
indicator_mapping = {"ir":"return", "r_a":"return_a", "er_a":"excess_a", "odds":"odds",
                     ...
                     "info_a":"in_a", "jensen_a":"jense_a", "treynor_a":"tr_a"}
...
for n in range(N1, N2):
    ...
    for i in range(len(ids_o)):
        ...
        i_r = map(nan2null, list(map(C.interval_return, nav)))  #
        r_a = map(nan2null, list(map(C.return_a, r, [12]*length_)))
        er_a = map(nan2null, list(map(C.excess_return_a, r, r_bm[:length_], [12]*length_)))
        #odds = map(lambda x: str(x), list(map(D.odds, r, r_bm, [12]*length_))[2:])
        #sharpe_a = map(lambda x: str(x), np.array((list(map(D.sharpe_a, r, r_f, [12]*10))))[rng_3].tolist())
        calmar_a = np.array(list(map(nan2null, map(C.calmar_a, nav_weighted, r_f[:length_], [12]*length_))))[rng].tolist()
        sortino_a = np.array(list(map(nan2null, map(C.sortino_a, r, r_f[:length_], [12]*length_))))[rng].tolist()
        info_a = np.array(list(map(nan2null, map(C.info_a, r, r_f[:length_], [12]*length_))))[rng].tolist()
        jensen_a = np.array(list(map(nan2null, map(C.jensen_a, r, r_bm[:length_], r_f[:length_], [12]*length_))))[rng].tolist()
        treynor_a = np.array(list(map(nan2null, map(C.treynor_a, r, r_bm[:length_], r_f[:length_], [12]*length_))))[rng].tolist()
