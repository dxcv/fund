...
for year in range(first_year, now.timetuple().tm_year+1):
    ...
    AND fund_id IN (SELECT fund_id FROM fund_type_mapping WHERE stype_code = 6010702) \
...
#op.to_csv("C:\\Users\\Kael Yu\\Desktop\\fund_index_03_m.csv", index=False)
...
for i in range(len(tag)):
    res_tmp = {"index_id": "FI03", "index_name":"私募FOF指数", "statistic_date":str(tag[i]), "update_time":now_str,
               "typestandard_code": "601", "typestandard_name": "按投资策略分类", "type_code": "60107", "type_name": "组合策略",
               "stype_code":"6010702", "stype_name":"FOF", "index_method": "1", "data_source":"0", "data_source_name":"私募云通",
