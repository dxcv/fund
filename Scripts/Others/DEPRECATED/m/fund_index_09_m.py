...
for year in range(first_year, now.timetuple().tm_year+1):
    ...
    sql_i = "\
    SELECT fund_id, nav, statistic_date FROM fund_nv_data WHERE source_code = 3 AND statistic_date < '{0}' and statistic_date >= '{1}' \
...
#op.to_csv("C:\\Users\\Kael Yu\\Desktop\\fund_index_09_m.csv", index=False)
