...
variable_name = ["stdev_a", "dstd_a", "mdd", "beta", "var"]
...
for n in range(N1,N2):
    ...
    for i in range(len(ids_o)):
        ...
        stdev_a = map(nan2null, map(C.standard_deviation_a, r))  #
        dstd_a = map(nan2null, map(C.downside_deviation_a, r, r_f))
        mdd = map(nan2null, map(lambda x: x[0],map(C.max_drawdown, nav_weighted)))
        beta = map(nan2null, map(C.beta, r, r_bm, r_f))
        var = map(nan2null, map(C.value_at_risk, r))

        res1 = [stdev_a, dstd_a, mdd, beta, var]
        #res2 = [odds]
