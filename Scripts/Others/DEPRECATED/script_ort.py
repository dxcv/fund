...
WHERE org_type_code = 1 \
    AND fund_id IN (SELECT fund_id FROM fund_info WHERE foundation_date <= '%s')" %(date_s - relativedelta(months=3))
...
for k in interval:
    ...
    r_f.append(list(map(annual2month, tbond_reshape))[:-1])
...
for n in range(N1, 1):
    ...
    sql3 = "\
    SELECT T2.org_id, T1.fund_id, T1.asset_scale, T1.statistic_date, T3.foundation_date \
    FROM fund_asset_scale as T1,\
	    (SELECT org_id, fund_id FROM fund_org_mapping WHERE org_id in %s) as T2, \
	    (SELECT fund_id, foundation_date FROM fund_info WHERE foundation_date <= '%s') as T3 \
    WHERE T1.fund_id = T2.fund_id AND T1.fund_id= T3.fund_id AND T1.statistic_date<= '%s' \
    ORDER BY org_id ASC, fund_id ASC, statistic_date DESC" % (id_generator(ids, n*100, (n+1)*100), (date_s - relativedelta(months=3)).strftime("%Y-%m-%d %H:%M:%S"), date_s.strftime("%Y-%m-%d %H:%M:%S"))
    ...
    data_asset = pd.read_sql(sql3, con=engine)
    data_asset.dropna(inplace=True)
    data_asset.index = range(len(data_asset))
    #list
