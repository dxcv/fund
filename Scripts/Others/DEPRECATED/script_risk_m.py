...
data = Data(statistic_date_s=date_s, start=0, end=len(ids), ids=ids, con=engine)
data.get_data()
...
#variable_name2 = ["odds"]
variable_name3 = ["stdev", "stdev_a", "dstd_a", "mdd", "beta", "corr"]
...
for i in range(len(fund_ids)):
    ...
    stdev_a = map(get_7f, list(map(D.standard_deviation_a, r, [12]*8)))
    dstd_a = map(get_7f, list(map(D.downside_deviation_a, r, r_f, [12]*8)))
    ...
    for res_seq, var in zip(res1, variable_name3):
        for res, int_ in zip(res_seq, interval_3):
