...
for date in range(1, 2):
    ...
    JOIN (SELECT fund_id, foundation_date \
          FROM fund_info \
          WHERE (data_freq = '周度' or data_freq = '周' or data_freq = '日度' or data_freq = '日') \
    ...
    r_bm = []
    for k in interval:
        t_match = TimeModule.match_timeseries_weekly(bm["statistic_date"].tolist(), k, date_s, end[k])
    ...
    for k in interval:
        t_match = TimeModule.match_timeseries_weekly(tbond["statistic_date"].tolist(), k, date_s, end[k])
    ...
    dlength = {"y":len(r_bm[0])+1, 3:len(r_bm[1])+1, 6:len(r_bm[2])+1,
               12:len(r_bm[3])+1, 24:len(r_bm[4])+1, 36:len(r_bm[5])+1, 60:len(r_bm[6])+1}


    #fund_ids = get_data.get_data["fund_id"].drop_duplicates().tolist()
    ...
    for n in range(N1, 1):
        ...
        t_matchs = [TimeModule.match_timeseries_weekly(x1,x2,x3,x4) for x1, x2, x3, x4 in zip(t_reals, ["y"]*length, [date_s]*length, [end["y"]]*length)]
        ...
        t_matchs = [TimeModule.match_timeseries_weekly(x1,x2,x3,x4) for x1, x2, x3, x4 in zip(t_reals, intervals_longest, [date_s]*length, ends_longest)]
        ...
        for i in range(1): #Calculate 1000 funds
            ...
            if len(datas) >= 2000:
                ...
                pl_result["result"] = str({"db_name":"base",
                                           "table_name":"fund_subsidiary_weekly_index",
                                           "param_fields":fields,
